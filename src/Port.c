...
#define TIEMPO_ESPERA_SPI 5	// 5 mSeg

#define CE_NRF24_Pin GPIO_PIN_7
#define CE_NRF24_GPIO_Port GPIOA

#define CSN_NRF24_Pin GPIO_PIN_15
#define CSN_NRF24_GPIO_Port GPIOA

/* Seteo de pines - Con macros facilita la portabilidad */
#define NRF24L01_CE_LOW		HAL_GPIO_WritePin(CE_NRF24_GPIO_Port, CE_NRF24_Pin, GPIO_PIN_RESET)
#define NRF24L01_CE_HIGH	HAL_GPIO_WritePin(CE_NRF24_GPIO_Port, CE_NRF24_Pin, GPIO_PIN_SET)
#define NRF24L01_CE_IS_HIGH	HAL_GPIO_ReadPin(CE_NRF24_GPIO_Port, CE_NRF24_Pin)

#define NRF24L01_CSN_HIGH 	HAL_GPIO_WritePin(CSN_NRF24_GPIO_Port, CSN_NRF24_Pin, GPIO_PIN_SET)
#define NRF24L01_CSN_LOW 	HAL_GPIO_WritePin(CSN_NRF24_GPIO_Port, CSN_NRF24_Pin, GPIO_PIN_RESET)

/* Defino una macro para la funcion Delay - Con macros facilita la portabilidad */
#define DELAY_MS(delay) HAL_Delay(delay)


#define NRF24L01_WR_REGISTER_MASK			0x1F  // 000x xxxx escribo o leo un registro
...
static void vNRF24L01_LeerComando(uint8_t *registro);
static void vNRF24L01_EscribirComando(uint8_t *registro);
...
void vNRF24L01_SPI1_CE_CSN_Iniciar(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(CSN_NRF24_GPIO_Port, CSN_NRF24_Pin, GPIO_PIN_SET);
...
static void vNRF24L01_LeerComando(uint8_t *registro){
	...
	*registro = NRF24L01_WR_REGISTER_MASK & *registro;
}
...
static void vNRF24L01_EscribirComando(uint8_t *registro){
	...
	*registro = 0x20 | (NRF24L01_WR_REGISTER_MASK & *registro);
}
...
uint8_t byNRF24L01_LeerRegistro(uint8_t registro) {
	...
	if(HAL_SPI_Transmit(&hspi1, &registro, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		// NRF24L01_NOP_MASK: 0xFF. No Operation. Might be used to read the STATUS register
		registro = NRF24L01_NOP_MASK;
		/* Leer el byte recibido */
		if(HAL_SPI_TransmitReceive(&hspi1, &registro, &byteLeido, sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK){
			SPI_Error();
		}
	}
	else{
...
void vNRF24L01_LeerMultiRegistros(uint8_t registro, uint8_t* vBytes, uint8_t cantidad) {
	...
	if(HAL_SPI_Transmit(&hspi1, &registro, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		...
		if(HAL_SPI_TransmitReceive(&hspi1, &registro, vBytes, cantidad * sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
		{
			SPI_Error();
		}
	}
...
void vNRF24L01_EscribirRegistro(uint8_t registro, uint8_t byteDato) {
	...
	if(HAL_SPI_Transmit(&hspi1, &registro, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		...
		if(HAL_SPI_Transmit(&hspi1, &byteDato, sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
		{
			SPI_Error();
		}
	}
	...
	byNRF24L01_LeerRegistro(registro & NRF24L01_WR_REGISTER_MASK); // Solo para Test....
...
void vNRF24L01_EscribirMultiRegistros(uint8_t registro, uint8_t *vBytes, uint8_t count) {
	...
	if(HAL_SPI_Transmit(&hspi1, &registro, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		...
		if(HAL_SPI_Transmit(&hspi1, vBytes, count*sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
		{
			SPI_Error();
		}
	}
...
void vNRF24L01_EscribirPayload(uint8_t *vBytes, uint8_t PayloadSize){
	...
	uint8_t byte_temp = NRF24L01_W_TX_PAYLOAD_MASK;
	/* Chip select nivel bajo - Selecciono el dispositivo para el SPI*/
	NRF24L01_CSN_LOW;
	/* Enviar el comando para escribir el payload de TX */
	if(HAL_SPI_Transmit(&hspi1, &byte_temp, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		/* Enviar los datos al payload*/
		if(HAL_SPI_Transmit(&hspi1, vBytes, PayloadSize*sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
		{
			SPI_Error();
		}
	}
...
void vNRF24L01_LeerPayload(uint8_t *vBytes, uint8_t PayloadSize){
	...
	uint8_t byte_temp = NRF24L01_R_RX_PAYLOAD_MASK;
	/* Chip select nivel bajo - Selecciono el dispositivo para el SPI*/
	NRF24L01_CSN_LOW;
	/* Enviar el comando para leer el payload de RX */
	if(HAL_SPI_Transmit(&hspi1, &byte_temp, sizeof(uint8_t), TIEMPO_ESPERA_SPI) == HAL_OK)
	{
		byte_temp = NRF24L01_NOP_MASK; // NRF24L01_NOP_MASK: No Operation. Might be used to read the STATUS register
		/* Leer el payload */
		if(HAL_SPI_TransmitReceive(&hspi1, &byte_temp, vBytes, PayloadSize * sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
		{
			SPI_Error();
		}
	}
...
void vNRF24L01_FlushTx(void){
	...
	uint8_t byte_comando = NRF24L01_FLUSH_TX_MASK;
	/* Chip select nivel bajo - Selecciono el dispositivo para el SPI*/
	NRF24L01_CSN_LOW;
	/* Envio el comando para que borre el FIFO de TX */
	if(HAL_SPI_Transmit(&hspi1, &byte_comando, sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
	{
		SPI_Error();
	}
	/* Deshabilito el dispositivo para el SPI */
...
/* Borra el FIFO de RX */
void vNRF24L01_FlushRx(void){
	uint8_t byte_comando = NRF24L01_FLUSH_RX_MASK;
	/* Chip select nivel bajo - Selecciono el dispositivo para el SPI*/
	NRF24L01_CSN_LOW;
...
uint8_t byNRF24L01_GetStatus(void) {
	...
	uint8_t status, byte_NOP = NRF24L01_NOP_MASK;
	/* Chip select nivel bajo - Selecciono el dispositivo para el SPI*/
	NRF24L01_CSN_LOW;
	/* El primer byte recibido es el de status (0x07) */
	if(HAL_SPI_TransmitReceive(&hspi1, &byte_NOP, &status, sizeof(uint8_t), TIEMPO_ESPERA_SPI) != HAL_OK)
	{
		SPI_Error();
	}
	/* Deshabilito el dispositivo para el SPI */
...
void vNRF24L01_RxModo(void) {
	...
	vNRF24L01_FlushRx();
	/* Seteo modo RX */
	vNRF24L01_EscribirBit(NRF24L01_REG_CONFIG, NRF24L01_PRIM_RX, 1);
	/* Comenzar a "escuchar" */
	NRF24L01_CE_HIGH;
...
void vNRF24L01_TxModo(void) {
	...
	vNRF24L01_EscribirBit(NRF24L01_REG_CONFIG, NRF24L01_PRIM_RX, 0);
...
uint8_t bNRF24L01_StbyModoIOn(void){
	...
	if(!bNRF24L01_LeerBit(NRF24L01_REG_CONFIG, NRF24L01_PWR_UP))
	{
...
uint8_t bNRF24L01_PowerUp(void){
	...
	vNRF24L01_EscribirBit(NRF24L01_REG_CONFIG, NRF24L01_PWR_UP, 1);
...
uint8_t bNRF24L01_PowerDown(void){
	...
	vNRF24L01_EscribirBit(NRF24L01_REG_CONFIG, NRF24L01_PWR_UP, 0);
	/* Chequeo que la escritura fue exitosa */
	if(!bNRF24L01_LeerBit( NRF24L01_REG_CONFIG, NRF24L01_PWR_UP) )
	{
